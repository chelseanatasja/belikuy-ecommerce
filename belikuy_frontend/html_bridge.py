...
def render_original_html(page_id, html_content, height=800):
    ...
    comp_dir = os.path.realpath(os.path.join(os.path.dirname(__file__), "st_components", page_id))
    os.makedirs(comp_dir, exist_ok=True)
    with open(os.path.join(comp_dir, "streamlit-component-lib.js"), "w", encoding="utf-8") as f:
        f.write(JS_LIB)
    final = html_content.replace("</body>", BRIDGE + "\n</body>", 1) if "</body>" in html_content else html_content + BRIDGE
...
def inject_localstorage_sync(html_content):
    ...
    if st.session_state.get('user'):
        u_str = json.dumps(st.session_state['user']).replace("'", "\\'").replace('"', '\\"')
