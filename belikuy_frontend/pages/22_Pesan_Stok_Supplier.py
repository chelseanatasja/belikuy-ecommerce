...
st.set_page_config(page_title="Pesan Stok Supplier", layout="wide", initial_sidebar_state="collapsed")
hide_streamlit_ui()
require_login()
...
if not suppliers:
    ...
    for s in suppliers:
        sel = "selected" if str(s['id']) == str(st.session_state.get('selected_supplier_id')) else ""
        supplier_options += f"<option value='{s['id']}' {sel}>{s['supply_company_name']}</option>"
...
if sid:
    ...
    if not catalog:
        ...
        for c in catalog:
            ...
            sc_text = []
            if c.get("size"): sc_text.append(f"Ukuran: {c['size']}")
            if c.get("color"): sc_text.append(f"Warna: {c['color']}")
    ...
    if action == "select_supplier":
        ...
        if items_str:
            ...
            if company_id:
                ...
                catalog_res, _ = get_api(f"suppliers/{st.session_state['selected_supplier_id']}/catalog")
                catalog = catalog_res.get("data", []) if catalog_res else []
                ...
                if status in (200, 201):
                    ...
                    st.error(f"Gagal memproses: {order_res.get('error') if order_res else 'Unknown'}")
            else:
