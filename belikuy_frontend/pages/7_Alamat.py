...
from utils import get_api, post_api, put_api, delete_api, require_login, hide_streamlit_ui
from html_bridge import render_original_html
from unified_navbar import inject_navbar, handle_global_action

st.set_page_config(page_title="BeliKuy - Manajemen Alamat", layout="wide", initial_sidebar_state="collapsed")
hide_streamlit_ui()
require_login()
...
with open(os.path.join(HTML_BASE, "address_management/code.html"), encoding='utf-8') as f:
    html = f.read()
...
html = re.sub(r'(<div class="grid grid-cols-1 lg:grid-cols-2 gap-lg">)(.*?)(</main>)', rf'\1{addr_cards}</div>\3', html, flags=re.DOTALL)
...
# Modify the button to open modal
html = re.sub(r'(<button class="inline-flex items-center gap-2 bg-gradient-to-r from-primary-container[^>]*?)(>)', r'\1 onclick="toggleModal()"\2', html)

# Modify modal HTML structure slightly for IDs
modal_html = html[html.find('<!-- Overlay Modal'):]
new_modal_html = re.sub(r'<div class="fixed inset-0[^>]*?>', lambda m: m.group(0).replace('class="', 'id="addressModal" class="').replace('items-center', 'items-start pt-32'), modal_html, count=1)
new_modal_html = new_modal_html.replace('id="setMain"', 'id="setMain"')
# Replace close button action
new_modal_html = re.sub(r'(<button class="absolute top-6 right-6[^>]*?)(>)', r'\1 onclick="toggleModal()" type="button"\2', new_modal_html)
# Replace cancel button action
new_modal_html = re.sub(r'(<button class="px-6 py-3 rounded-full font-label-caps text-label-caps text-on-surface-variant[^>]*?)(>)', r'\1 onclick="toggleModal()" type="button"\2', new_modal_html)
# Replace save button action
new_modal_html = re.sub(r'(<button class="[^"]*bg-primary text-on-primary[^"]*")( type="button">|>)', r'\1 onclick="saveAddress()" type="button">', new_modal_html)

# Set input IDs
new_modal_html = new_modal_html.replace('placeholder="Cth: Rumah, Kantor"', 'placeholder="Cth: Rumah, Kantor" id="new_city"')
new_modal_html = new_modal_html.replace('placeholder="Nama jalan, gedung, no. rumah"', 'placeholder="Nama jalan, gedung, no. rumah" id="new_addr"')
...
if action_data:
    ...
    if handle_global_action(st, act, action_data, current_user):
        ...
        if addr and city:
            post_api(f"addresses", {"user_id": user['id'], "address": addr, "city": city, "postal_code": postal, "is_default": is_def})
            st.rerun()
        else:
            st.error("Alamat dan kota harus diisi")
    elif act == "set_default":
        aid = action_data.get("aid")
        if aid: put_api(f"addresses/{aid}/default", {"user_id": user['id']})
        st.rerun()
    elif act == "delete_address":
        aid = action_data.get("aid")
        if aid: delete_api(f"addresses/{aid}")
        st.rerun()
