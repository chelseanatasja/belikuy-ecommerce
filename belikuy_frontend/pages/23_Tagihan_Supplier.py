...
st.set_page_config(page_title="Tagihan Supplier", layout="wide", initial_sidebar_state="collapsed")
hide_streamlit_ui()
require_login()
...
paid_orders = [o for o in orders if o['status'] in ['paid', 'shipped', 'completed', 'cancelled']]
...
if action_data:
    ...
    if action == "pay_bill":
        ...
        order_id = action_data.get("order_id")
        res, status = put_api(f"suppliers/orders/{order_id}/receive", {"company_id": company_id})
        if status in (200, 201):
            st.session_state['_supplier_receive_success'] = True
            st.rerun()
        else:
            st.session_state['_supplier_receive_error'] = res.get("error") if res else "Unknown Error"
            st.rerun()
    elif action == "logout":
