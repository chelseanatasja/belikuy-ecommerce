...
st.set_page_config(
    page_title="BeliKuy - Laporan Pendapatan",
    layout="wide",
...
with open(
    os.path.join(HTML_BASE, "seller_income_report/code.html"), encoding="utf-8"
) as f:
    html = f.read()
...
html = re.sub(
    ...
    rf"\g<1>{format_rupiah(saldo_saat_ini)}\2",
    html,
)
...
# Saldo subtext
html = html.replace(
    "Siap ditarik ke rekening utama",
...
status_map = {
    "pending": (
        '<span class="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-yellow-100 text-yellow-700 font-label-caps text-[10px]"><span class="w-1.5 h-1.5 rounded-full bg-yellow-400"></span>Menunggu</span>'
    ),
    "processed": (
        '<span class="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-tertiary-container/40 text-tertiary font-label-caps text-[10px]"><span class="w-1.5 h-1.5 rounded-full bg-tertiary"></span>Diproses</span>'
    ),
    "completed": (
        '<span class="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-primary-container/30 text-on-primary-container font-label-caps text-[10px]"><span class="w-1.5 h-1.5 rounded-full bg-primary"></span>Selesai</span>'
    ),
    "rejected": (
        '<span class="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-error-container/50 text-error font-label-caps text-[10px]"><span class="w-1.5 h-1.5 rounded-full bg-error"></span>Ditolak</span>'
    ),
}
...
if action_data:
    ...
    if handle_seller_global_action(st, act):
        ...
        if amount and bank and account and holder:
            ...
            if code == 201:
                st.success(
                    "✅ Permintaan tarik dana berhasil diajukan! Akan diproses dalam 1-3 hari kerja."
                )
            else:
