...
st.set_page_config(page_title="BeliKuy - Manajemen Vendor", layout="wide", initial_sidebar_state="collapsed")
hide_streamlit_ui()
require_role("admin")
...
if st.session_state.search_query:
    ...
    vendors = [v for v in vendors if q in str(v.get("company_name", "")).lower() or q in str(v.get("username", "")).lower() or q in str(v.get("email", "")).lower()]

HTML_BASE = r"D:\belikuy\belikuy_ui_templates"
with open(os.path.join(HTML_BASE, "super_admin_vendor_management/code.html"), encoding='utf-8') as f:
    html = f.read()
...
for v in vendors:
    ...
    status_badge = '<span class="inline-flex items-center px-3 py-1 rounded-full bg-primary-fixed text-on-primary-fixed-variant font-label-caps text-label-caps uppercase shadow-[0_2px_10px_rgba(255,217,222,0.5)]">Aktif</span>' if is_active else '<span class="inline-flex items-center px-3 py-1 rounded-full bg-surface-variant text-on-surface-variant font-label-caps text-label-caps uppercase">Nonaktif</span>'
    ...
    toggle_color = "text-on-surface-variant hover:bg-surface-variant" if is_active else "text-emerald-600 hover:bg-emerald-100"
...
html = re.sub(r'(<tbody class="divide-y divide-surface-variant">)(.*?)(</tbody>)', rf'\1{rows}\3', html, flags=re.DOTALL)
html = re.sub(r'(Menampilkan 1-3 dari 124 vendor)', rf'Total {len(vendors)} vendor', html)
...
# Inject search value
if st.session_state.search_query:
    html = html.replace('placeholder="Cari nama toko atau SKU..."', f'value="{st.session_state.search_query}" placeholder="Cari nama toko atau SKU..."')
...
if action_data:
    ...
    if handle_admin_global_action(st, act):
        ...
        try:
            conn = mysql.connector.connect(host="127.0.0.1", user="root", password="", database="belikuy")
            cursor = conn.cursor()
            cursor.execute("UPDATE belikuy_seller_db.Companies SET is_active = NOT is_active WHERE id = %s", (vid,))
            cursor.execute("UPDATE companies SET is_active = NOT is_active WHERE id = %s", (vid,))
            conn.commit()
