...
st.set_page_config(
    page_title="BeliKuy - Seller Dashboard",
    layout="wide",
...
pesanan_baru = sum(
    ...
    if str(o.get("order_status", o.get("status", ""))).lower() == "paid"
)
produk_aktif = sum(
    1 for p in products if p.get("is_active", 1) and p.get("stock", 0) > 0
)
...
for o in recent_display:
    img = (
        get_image_base64(o.get("image_url"))
        if o.get("image_url")
        else "https://via.placeholder.com/100?text=Produk"
    )
...
if not recent_orders_html:
    recent_orders_html = (
        '<p class="text-sm text-outline p-4 text-center">Belum ada pesanan terbaru.</p>'
    )
...
if monthly_omzet:
    min_year = min(
        int(row.get("month", "").split("-")[0])
        for row in monthly_omzet
        if row.get("month")
    )
    max_year = max(
        int(row.get("month", "").split("-")[0])
...
if not monthly_omzet:
    ...
    omzet_dict = {
        row.get("month", ""): float(row.get("omzet", 0)) for row in monthly_omzet
    }

    display_months = []
    for cm in continuous_months:
        display_months.append({"month": cm, "omzet": omzet_dict.get(cm, 0)})
...
html = re.sub(
    r'(<h3 class="font-h3 text-h3 text-on-background">)(42)(</h3>)',
    rf"\g<1>{pesanan_baru}\3",
    html,
)
html = re.sub(
    r'(<h3 class="font-h3 text-h3 text-on-background">)(156)(</h3>)',
    rf"\g<1>{produk_aktif}\3",
    html,
)
html = re.sub(
    r'(<h3 class="font-h3 text-h3 text-on-background">)(4\.9)(<span class="text-sm text-on-surface-variant font-normal">/5\.0</span></h3>)',
    rf"\g<1>{rating_toko:.1f}\3",
    html,
)

# Remove mock data & replace with real subtexts
html = re.sub(
    r"<p[^>]*>\s*<span[^>]*>trending_up</span> \+15% dari bulan lalu\s*</p>",
    f'<p class="font-body-sm text-sm text-on-surface-variant mt-1 flex items-center gap-1">Dari {total_pesanan_selesai} pesanan selesai</p>',
    html,
)
...
html = re.sub(
    r"<p[^>]*>\s*Berdasarkan 320 ulasan\s*</p>",
    f'<p class="font-body-sm text-sm text-on-surface-variant mt-1">Rating toko kamu</p>',
    html,
)

# Replace Recent Orders Content
html = re.sub(
    r'(<div class="flex flex-col gap-4 flex-1">)(.*?)(</div>\s*<button)',
    rf"\1{recent_orders_html}\3",
    html,
    flags=re.DOTALL,
)
...
html = re.sub(
    r'(<div class="relative z-10 w-full flex justify-between items-end h-full group">)(.*?)(</div>\s*</div>\s*</div>)',
    rf"\1\n{chart_html}\n\3",
    html,
    flags=re.DOTALL,
)
...
html = re.sub(
    ...
    lambda m: m.group(0).replace(
        "<button", "<button onclick=\"stNavigate({action:'go_orders'})\"", 1
    ),
    html,
)
