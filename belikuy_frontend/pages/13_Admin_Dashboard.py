...
st.set_page_config(page_title="BeliKuy - Admin Dashboard", layout="wide", initial_sidebar_state="collapsed")
hide_streamlit_ui()
require_login()
...
try:
    ...
    cursor.execute("SELECT COUNT(*) as c, SUM(total_price) as s FROM orders WHERE status != 'cancelled' AND created_at >= DATE_SUB(NOW(), INTERVAL 30 DAY)")
    month_stats = cursor.fetchone() or {'c': 0, 's': 0}
    
    cursor.execute("SELECT COUNT(*) as c, SUM(total_price) as s FROM orders WHERE status != 'cancelled'")
    ...
    for idx, row in enumerate(chart_data):
        ...
        bg_class = "bg-primary" if pct == 100 else "bg-primary-container/60 hover:bg-primary/80 transition-colors cursor-pointer"
    ...
    cursor.execute("SELECT id, total_price, created_at, 'order' as type FROM orders ORDER BY created_at DESC LIMIT 5")
    ro = cursor.fetchall()
    cursor.execute("SELECT id, company_name as name, created_at, 'vendor' as type FROM belikuy_seller_db.companies ORDER BY created_at DESC LIMIT 5")
    rv = cursor.fetchall()
...
with open(os.path.join(HTML_BASE, "super_admin_dashboard_overview/code.html"), encoding='utf-8') as f:
    html = f.read()
...
def time_ago(dt):
    ...
    if diff < 3600: return f"{int(diff/60)} menit yang lalu"
    if diff < 86400: return f"{int(diff/3600)} jam yang lalu"
    return f"{int(diff/86400)} hari yang lalu"

activity_html = ""
for a in recent_activities:
    icon = "store" if a['type'] == 'vendor' else "monetization_on"
    color = "bg-tertiary-container/30 text-tertiary border-surface-container-lowest" if a['type'] == 'vendor' else "bg-emerald-100 text-emerald-700 border-surface-container-lowest"
    title = "Toko Baru Mendaftar" if a['type'] == 'vendor' else "Pesanan Baru"
    desc = f"Vendor '{a.get('name','')}' bergabung." if a['type'] == 'vendor' else f"Order #{a['id']} senilai {format_rupiah(a['total_price'])} diproses."
...
html = re.sub(r'(<h3 class="font-h3 text-\[20px\] font-semibold text-on-background mb-6">Aktivitas Terbaru</h3>\s*<div class="flex flex-col gap-6">).*?(</div>\s*<button)', rf'\1{activity_html}\2', html, flags=re.DOTALL)

# Replace Chart
html = re.sub(r'(<!-- Mock Graph Area \(Stylized\) -->).*?(</section>)', rf'\1\n{chart_html}\n\2', html, flags=re.DOTALL)
...
html = re.sub(r'<button class="flex items-center gap-2 text-sm font-medium text-on-surface-variant hover:text-primary transition-colors px-3 py-1.5 rounded-lg border border-surface-variant hover:bg-surface-bright">\s*Minggu Ini\s*<span class="material-symbols-outlined text-\[18px\]">expand_more</span>\s*</button>', dropdown_html, html)
