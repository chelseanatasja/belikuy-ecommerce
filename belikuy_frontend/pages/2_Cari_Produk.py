...
st.set_page_config(
    page_title="BeliKuy - Cari Produk", layout="wide", initial_sidebar_state="collapsed"
)
hide_streamlit_ui()
require_login()
...
if q:
    api_p["search"] = q
if cat:
    api_p["category_id"] = cat
if min_price:
    api_p["min_price"] = min_price
if max_price:
    api_p["max_price"] = max_price
...
def card(p):
    ...
    rhtml = (
        f'<div class="flex items-center gap-1 mb-1 text-primary-container">'
    ...
    return (
        f'<div class="bg-white rounded-2xl p-4 shadow-[0_8px_30px_rgb(255,182,193,0.08)] '
...
products_html = (
    "".join(card(p) for p in products_paged)
    if products_paged
    else (
...
page_info = (
    f'<span class="font-body-md text-on-surface">Page {page} of {total_pages}</span>'
)
...
for c in categories:
    ...
    cat_items += (
        f'<label class="flex items-center gap-3 cursor-pointer group">'
...
with open(
    os.path.join(HTML_BASE, "search_filter_page/code.html"), encoding="utf-8"
) as f:
    html = f.read()
...
html = re.sub(
    r'Showing <strong[^>]*>\d+</strong> results for "Kawaii Accessories"', info, html
)
...
for lbl in ["Categories", "Flash Sale", "Brands"]:
    html = html.replace(
        f">{lbl}</a>",
        f" onclick=\"event.preventDefault();stNavigate({{action:'filter',q:'',cat:''}})\">{lbl}</a>",
        1,
    )
...
if action_data:
    ...
    if handle_global_action(st, act, action_data, current_user):
        ...
        if nq:
            st.query_params["q"] = nq
        if nc:
            st.query_params["cat"] = nc
        if nmin:
            st.query_params["min_price"] = nmin
        if nmax:
            st.query_params["max_price"] = nmax
        if nsrt and nsrt != "newest":
            st.query_params["sort"] = nsrt
