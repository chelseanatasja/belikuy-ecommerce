...
st.set_page_config(page_title="BeliKuy - Checkout", layout="wide", initial_sidebar_state="collapsed")
hide_streamlit_ui()
require_login()
...
def local_img(path):
    if not path: return "https://via.placeholder.com/150?text=No+Image"
...
if not shipment_cos: shipment_cos = []
...
html = re.sub(
    ...
    rf'\1{addr_html}\3', html, flags=re.DOTALL
)
...
html = re.sub(
    ...
    rf'\1{items_html}\3', html, flags=re.DOTALL
)
...
html = re.sub(
    ...
    rf'\1{ship_html}\3', html, flags=re.DOTALL
)
...
if payment_methods:
    ...
    for idx3, p in enumerate(payment_methods):
        ...
        if p['institution_type'] == 'Fintech': icon = "account_balance_wallet"
        elif p['institution_type'] == 'Virtual Account': icon = "credit_card"
...
html = re.sub(
    ...
    rf'\1{pay_html}\3', html, flags=re.DOTALL
)
...
if action_data:
    ...
    if handle_global_action(st, act, action_data, current_user):
        ...
        res, status = post_api("orders/checkout", {
            "user_id":           user['id'],
        ...
        if status == 201:
            ...
            chosen_method = next(
                (m for m in payment_methods if str(m.get('id')) == str(payment_method_id)),
                None
            )
            ...
            st.session_state['pending_payment_type'] = (
                chosen_method.get('institution_type', 'transfer') if chosen_method else 'transfer'
            )
