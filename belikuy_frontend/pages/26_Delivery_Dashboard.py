...
st.set_page_config(
    page_title="Delivery Dashboard", layout="wide", initial_sidebar_state="collapsed"
)
hide_streamlit_ui()
require_login()
...
def get_delivery_db():
    return mysql.connector.connect(
        host="127.0.0.1", user="root", password="", database="belikuy_delivery_db"
    )
...
cursor.execute(
    "SELECT * FROM shipment_companies WHERE user_id = %s LIMIT 1", (user["id"],)
)
...
total_in_transit = sum(
    1
    for s in shipments
    if s["shipping_status"] in ("shipped", "in_transit", "picked_up")
)
...
html = html.replace(
    "{TABLE_HEADERS}",
    "<tr><th class='p-4'>Resi</th><th class='p-4'>Tanggal</th><th class='p-4'>Ongkir</th><th class='p-4'>Status</th><th class='p-4 text-right'>Aksi</th></tr>",
)
...
if not shipments:
    html = html.replace(
        "{EMPTY_STATE}",
        "<div class='p-8 text-center text-on-surface-variant'>Belum ada paket untuk dikirim.</div>",
    )
else:
...
html = inject_custom_sidebar(
    html, "Delivery", "Ekspedisi", "local_shipping", user["username"][0].upper()
)
...
if action:
    if action.get("action") == "deliver_order":
        ...
        cur.execute(
            "UPDATE shipments SET shipping_status = 'delivered' WHERE id = %s", (sid,)
        )
        # Get order_id
        ...
        if shipment and shipment.get("order_id"):
            m_conn = mysql.connector.connect(
                ...
                user="root",
                password="",
                database="belikuy_marketplace_db",
            )
            m_cur = m_conn.cursor()
            m_cur.execute(
