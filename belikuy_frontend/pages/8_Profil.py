...
st.set_page_config(page_title="BeliKuy - Profil Saya", layout="wide", initial_sidebar_state="collapsed")
hide_streamlit_ui()
require_login()
...
with open(os.path.join(HTML_BASE, "user_profile_page/code.html"), encoding='utf-8') as f:
    html = f.read()

# Dynamic data injection
html = html.replace('value="Amanda Putri"', f'id="inp_name" value="{username}"')
html = html.replace('value="amanda.putri@example.com"', f'id="inp_email" value="{email}"')
...
if gender == 'male':
    ...
    html = html.replace('checked="" class="text-primary focus:ring-primary-container" name="gender" type="radio" value="female"', 'id="gender_female" class="text-primary focus:ring-primary-container" name="gender" type="radio" value="female"')
else:
    html = html.replace('value="male"', 'id="gender_male" value="male"')
    html = html.replace('checked="" class="text-primary focus:ring-primary-container" name="gender" type="radio" value="female"', 'id="gender_female" checked class="text-primary focus:ring-primary-container" name="gender" type="radio" value="female"')
...
html = re.sub(r'<nav class="flex flex-col gap-sm">.*?</nav>', sidebar_replacement, html, flags=re.DOTALL)
...
if action_data:
    ...
    if handle_global_action(st, act, action_data, current_user):
        ...
        res, status = put_api(f"auth/profile/{user['id']}", {
