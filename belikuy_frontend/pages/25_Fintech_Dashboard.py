...
st.set_page_config(
    page_title="Fintech Dashboard", layout="wide", initial_sidebar_state="collapsed"
)
hide_streamlit_ui()
require_login()
...
def get_payment_db():
    return mysql.connector.connect(
        host="127.0.0.1", user="root", password="", database="belikuy_fintech_db"
    )
...
cursor.execute(
    "SELECT * FROM payment_methods WHERE user_id = %s LIMIT 1", (user["id"],)
)
...
total_processed = sum(
    [float(t["amount"]) for t in transactions if t["status"] in ("paid", "success")]
)
...
html = html.replace(
    "{TABLE_HEADERS}",
    "<tr><th class='p-4'>Mutasi ID</th><th class='p-4'>Tipe/Bank</th><th class='p-4'>Tanggal</th><th class='p-4'>Jumlah</th><th class='p-4'>Status</th></tr>",
)
...
if not transactions:
    html = html.replace(
        "{EMPTY_STATE}",
        "<div class='p-8 text-center text-on-surface-variant'>Belum ada transaksi B2C/B2B masuk.</div>",
    )
else:
...
html = inject_custom_sidebar(
    html, "Fintech", "Payment", "account_balance", user["username"][0].upper()
)
