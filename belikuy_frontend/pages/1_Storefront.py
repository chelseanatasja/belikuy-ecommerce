...
st.set_page_config(page_title="BeliKuy - Beranda", layout="wide", initial_sidebar_state="collapsed")
hide_streamlit_ui()
...
with open(os.path.join(HTML_BASE, "homepage_storefront/code.html"), encoding='utf-8') as f:
    html = f.read()
...
pattern_products = r'(<div class="grid grid-cols-2 md:grid-cols-4 gap-6">)(.*?)(</div>\s*</section>)'
if dynamic_products_html:
    html = re.sub(pattern_products, rf'\1\n{dynamic_products_html}\n\3', html, flags=re.DOTALL)
...
if not categories_db: categories_db = []
    ...
    if handle_global_action(st, act, action_data, user):
        ...
        if pid:
            ...
            if prod:
                ...
                existing = next((x for x in st.session_state['cart'] if str(x.get('id')) == str(pid)), None)
                if existing:
