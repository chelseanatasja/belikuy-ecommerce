...
const getProducts = async (filters) => {
    ...
    if (category_id) {
        const catIds = String(category_id).split(',').map(id => parseInt(id.trim(), 10)).filter(id => !isNaN(id));
        if (catIds.length > 0) {
...
const getProductReviews = async (productId) => {
    const [reviews] = await db.query(`
        ...
        FROM Reviews r
        LEFT JOIN belikuy_marketplace_db.Users u ON r.user_id = u.id
        WHERE r.product_id = ?
...
const getProductsByIds = async (productIds) => {
    ...
    const [rows] = await db.query(`
        ...
        FROM belikuy_seller_db.Products p
...
const deductStock = async (items) => {
    for (const item of items) {
        await db.query("UPDATE belikuy_seller_db.Products SET stock = stock - ? WHERE id = ?", [item.quantity, item.product_id]);
    }
    return { success: true };
};

module.exports = { getProducts, getProductById, getProductsByCompany, createProduct, updateProduct, toggleProductActive, deleteProduct, getCategories, getProductReviews, getProductsByIds, deductStock };
