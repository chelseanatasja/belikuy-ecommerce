...
const addShipmentCompany = async (companyName, serviceType) => {
    const [result] = await db.query('INSERT INTO Shipment_Companies (company_name, service_type) VALUES (?, ?)', [companyName, serviceType]);
    return { id: result.insertId };
};

const updateShipmentCompany = async (id, companyName, serviceType) => {
    await db.query('UPDATE Shipment_Companies SET company_name = ?, service_type = ? WHERE id = ?', [companyName, serviceType, id]);
    return true;
};

const deleteShipmentCompany = async (id) => {
    await db.query('DELETE FROM Shipment_Companies WHERE id = ?', [id]);
    return true;
};

const getSupplyCompanies = async () => {
    const [rows] = await db.query('SELECT * FROM Supply_Companies ORDER BY supply_company_name ASC');
    return rows;
};

const addSupplyCompany = async (name, contact, address) => {
    const [result] = await db.query('INSERT INTO Supply_Companies (supply_company_name, contact_number, address) VALUES (?, ?, ?)', [name, contact, address]);
    return { id: result.insertId };
};

const updateSupplyCompany = async (id, name, contact, address) => {
    await db.query('UPDATE Supply_Companies SET supply_company_name = ?, contact_number = ?, address = ? WHERE id = ?', [name, contact, address, id]);
    return true;
};

const deleteSupplyCompany = async (id) => {
    await db.query('DELETE FROM Supply_Companies WHERE id = ?', [id]);
    return true;
};
...
const getStats = async () => {
    ...
    // Verified Companies
    const [[totalCompanies]] = await db.query('SELECT COUNT(*) AS count FROM Companies'); // We assume all are verified for now or add is_verified if exists. Wait, Belikuy schema has no is_verified? Let's check. Actually just return totalCompanies for now.
    ...
    const [recentVendors] = await db.query('SELECT id, company_name, created_at, "vendor" as type FROM Companies ORDER BY created_at DESC LIMIT 3');
